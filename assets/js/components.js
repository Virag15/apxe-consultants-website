...
// ============================================================
(function () {
  // Folder-style URLs. /about/ serves /about/index.html natively on any host.
  var path = function (p) {
    if (!p || p === '/') return '/';
  ...
  var isActive = function (key) {
    if (key === 'home' && (pathname === '/' || pathname === '/index.html')) return 'is-active';
    if (key === 'about' && pathname.indexOf('/about') === 0) return 'is-active';
    if (key === 'services' && inServices) return 'is-active';
    if (key === 'work' && inWork) return 'is-active';
    if (key === 'contact' && pathname.indexOf('/contact') === 0) return 'is-active';
    return '';
  };
